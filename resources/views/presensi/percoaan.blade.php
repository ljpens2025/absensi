...
@else
    ...
    <script>
        ...
        const KANTOR_LAT = -7.390789267509395; // Ganti dengan latitude kantor
        const KANTOR_LNG = 112.51373249264314; // Ganti dengan longitude kantor
        ...
        $(document).ready(function() {
            $('.tampilkanpeta').on('click', function() {
                ...
                if (jarak <= RADIUS_KANTOR) {
                    statusBadge.removeClass('bg-danger').addClass('bg-success').text('Dalam Radius Kantor');
                } else {
                    statusBadge.removeClass('bg-success').addClass('bg-danger').text('Luar Radius Kantor');
                }
        ...
        function initMap() {
            // Inisialisasi peta dengan pusat di kantor
            map = L.map('mapContainer').setView([KANTOR_LAT, KANTOR_LNG], 16);
        ...
        function updateMap(lat, lng) {
            // Hapus marker karyawan lama
            if (markerKaryawan) {
                map.removeLayer(markerKaryawan);
            }
            
            // Tambahkan marker karyawan baru
            const jarak = hitungJarak(KANTOR_LAT, KANTOR_LNG, lat, lng);
            ...
            // Zoom ke area yang sesuai
            const group = new L.featureGroup([markerKantor, markerKaryawan]);
